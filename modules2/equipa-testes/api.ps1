    ...
    switch ($Action) {
        ...
        "addMissingGroups" {
            ...
            if ([string]::IsNullOrWhiteSpace($userId)) {
                Send-Json @{ success = $false; message = "userId não informado." }
                return
            }

            if (-not $groups -or $groups.Count -eq 0) {
                Send-Json @{ success = $false; message = "Nenhum grupo informado." }
                return
            }
            ...
            Send-Json @{
                ...
                message = if ($errors.Count -eq 0) { "Grupos adicionados com sucesso." } else { "Alguns grupos não foram adicionados." }
                added = $added
                errors = $errors
            }
            return
