...
function Build-Diagnosis {
    ...
    $comparison.Add((New-ComparisonRow 'UPN' $Ad.userPrincipalName $OnPrem.primarySmtpAddress $Entra.userPrincipalName $Exo.primarySmtpAddress (
        if ($Ad.userPrincipalName -and $Entra.userPrincipalName -and $Ad.userPrincipalName -ieq $Entra.userPrincipalName) {'OK'} else {'AVISO'}
    )))
    $comparison.Add((New-ComparisonRow 'ExchangeGuid' $adGuid $onPremGuid '—' $exoGuid (
        if ($guidStatus -eq 'GUIDs iguais') {'OK'} elseif ($guidStatus -match 'divergentes|vazio') {'ERRO'} else {'AVISO'}
    )))
    $comparison.Add((New-ComparisonRow 'ArchiveGuid' $Ad.archiveGuid $OnPrem.archiveGuid '—' $Exo.archiveGuid (
        if ($OnPrem.archiveGuid -and $Exo.archiveGuid -and $OnPrem.archiveGuid -ieq $Exo.archiveGuid) {'OK'} elseif (-not $OnPrem.archiveGuid -and -not $Exo.archiveGuid) {'OK'} else {'AVISO'}
    )))
    $comparison.Add((New-ComparisonRow 'RecipientTypeDetails' $Ad.recipientTypeDetails $OnPrem.recipientTypeDetails '—' $Exo.recipientTypeDetails (
        if ($OnPrem.hasRemoteMailbox) {'OK'} else {'AVISO'}
    )))
    $comparison.Add((New-ComparisonRow 'RemoteRoutingAddress' $Ad.targetAddress $OnPrem.remoteRoutingAddress '—' $Exo.externalEmailAddress (
        if ($Ad.targetAddress -or $OnPrem.remoteRoutingAddress) {'OK'} else {'AVISO'}
    )))
    $comparison.Add((New-ComparisonRow 'ImmutableId' $Ad.expectedImmutableId '—' $Entra.onPremisesImmutableId '—' (
        if ($Ad.expectedImmutableId -and $Entra.onPremisesImmutableId -and $Ad.expectedImmutableId -eq $Entra.onPremisesImmutableId) {'OK'} else {'AVISO'}
    )))
