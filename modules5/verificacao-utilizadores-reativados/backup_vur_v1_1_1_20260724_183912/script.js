(function () {
    ...
    async function loadStatus() {
        ...
        try {
            ...
            console.error(
                "[VUR V1.1.0] Erro ao verificar ligações",
                error
            );
    ...
    function renderHybridValidation(result) {
        var panel = byId("vurHybridPanel");
        var host = byId("vurHybridChecks");
        var classification =
            byId("vurClassification");
        var actionsHost =
            byId("vurRecommendedActions");
        ...
        if (diagnosis.status === "Com erro") {
            classification.classList.add(
                "error"
            );
        } else if (
            diagnosis.status === "Pendente"
        ) {
            classification.classList.add(
                "pending"
            );
        } else if (
            diagnosis.status === "Com avisos"
        ) {
            classification.classList.add(
                "warning"
            );
        ...
        checks.forEach(function (check) {
            ...
            item.innerHTML =
                '<div class="vur-check-icon">' +
                symbol +
                "</div>" +
                '<div class="vur-check-content">' +
                "<strong>" +
    ...
    async function verifyUser() {
        console.log(
            "[VUR V1.1.0] Clique em verificar utilizador"
        );
        ...
        try {
            ...
            console.error(
                "[VUR V1.1.0] Erro no diagnóstico",
                error
            );
    ...
    if (!state.initialized) {
        ...
        console.log(
            "[VUR V1.1.0] Delegação global instalada."
        );
    }
    ...
    function waitForModule() {
        ...
        function check() {
            ...
            if (attempts < maxAttempts) {
                ...
                console.warn(
                    "[VUR V1.1.0] Página do módulo não foi localizada."
                );
            }
        }
    ...
    console.log(
        "[VUR V1.1.0] script.js executado."
    );
})();